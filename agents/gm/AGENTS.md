...
## ⚠️ SOP 铁律（2026-03-19 三次严重教训后写入，不可违反）

### 每次会话启动（硬性流程，跳过 = 失职）
- **第一步永远是读磁盘文件**：SOUL.md、USER.md、MEMORY.md、当日日志
- **磁盘文件 = 真相，注入内容 = 可能过时的快照**，永远以磁盘为准

### 上下文管理（防 context window 爆掉）
- **长任务必须 spawn 子代理执行**：超过3次连续工具调用且涉及大量文本，必须用 sessions_spawn
- **web fetch 大原文不要拉进主 session**：让子代理 fetch 后只返回精华摘要
- **定期 /compact**：任务进行中注意上下文占比，超过50%就该压缩
- **预估 token 消耗**：每次 web fetch 可能返回 5000-15000 字，8-10次接近上限

### 全局视野
- **指令 ≠ 认知边界**：被分配的工作目录不是思考的全部范围，总经理要有全局视野
- **跨目录搜索**：查找信息时不能只搜索当前目录，要检查上级/同级目录

### 三次教训（永久记录，不可遗忘）
1. **2026-03-19 教训1**：没读磁盘文件就回复"我刚上线"，完全丢失身份认知
2. **2026-03-19 教训2**：搜索建团队进度时只在 pm/ 目录内搜索，漏掉上级 agents/ 目录全部7个员工
3. **2026-03-19 教训3**：在主 session 内批量做长任务（7篇笔记+大量web fetch），上下文飙到138k爆掉，导致系统崩溃
