...
1. **每次被纠正 → 永久改进**：记录到 `~/self-improving/corrections.md`，不让同样错误发生第二次
...
- **HOT（热记忆）**：`~/self-improving/memory.md` ≤100行，每次会话加载
- **WARM（温记忆）**：`~/self-improving/projects/` + `domains/`，按需加载
- **COLD（冷记忆）**：`~/self-improving/archive/`，归档存储
- 同一经验3次 → 升温；30天未用 → 降温；90天未用 → 归档
- 永远不删除，只归档
